...
import './DirectorViewFinal.css'

interface DirectorViewFinalProps {
  sessionId: string
  username: string
  difficulty: number
}
...
function DirectorViewFinal({ sessionId, username, difficulty }: DirectorViewFinalProps) {
  ...
  const fetchWavePreviews = async () => {
    // Simulate wave shots to show preview
    // This would call the backend /shoot endpoint for each edge position
    ...
    for (const pos of positions) {
      try {
        const response = await axios.post(
          ...
          { entry_position: pos },
          { params: { username: 'preview_' + username } }
        )
        previews.push({
          entry: pos,
      ...
      <div className="director-final">
        ...
        <div className="header-final">
          ...
          <div className="progress-final">
            ...
            <div className="progress-bar-final" style={{ width: `${(placedPieces.length / difficulty) * 100}%` }} />
          </div>
        </div>
        ...
        <div className="content-final">
          ...
          <div className="pieces-panel">
            <h3>Pieces</h3>
            <div className="pieces-grid-final">
              {PIECE_DEFINITIONS.map((piece) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`piece-card-final ${isPlaced ? 'placed' : ''} ${isSelected ? 'selected' : ''}`}
                    onClick={() => {
                      if (!isPlaced) {
                  ...
                  >
                    ...
                    <div className="piece-info-final">
          ...
          <div className="preview-panel-final">
            ...
            <div className="wave-previews">
              {wavePreviews.length > 0 ? (
                ...
                <div className="empty-preview-final">
...
export default DirectorViewFinal
